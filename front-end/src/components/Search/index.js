...
class Search extends Component {
    ...
    componentWillMount() {
        requestData();
        setTimeout(() => {
            // console.log(this.props.contactar);
            if (this.props.contactar) {
                this.setState({
    ...
    contactsInject() {
        ...
        if (this.state.showedUser !== null) {
            return this.state.showedUser.map(el => {
                ...
                el = el[1];
                return (
                    <List.Item key={key} onClick={this.onItemClick.bind(this)} value={key.toString()}>
                        <List.Content>
    ...
    render() {
        return (
            <section className="search">
                <Input placeholder="Search..." icon="search" fluid onInput={this.handleSearch.bind(this)} />
